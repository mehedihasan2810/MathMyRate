...
- Public routes are `/`; `/freelance/` plus the hourly rate, project rate, retainer, markup and margin, and salary to hourly calculators; `/fees/` plus the Stripe, PayPal, Square, Etsy, Gumroad, and Lemon Squeezy calculators and the `/fees/stripe-vs-paypal-fees/` and `/fees/gumroad-vs-lemon-squeezy-fees/` comparisons; `/guides/` and its guides; noindex `/embed/` copies of the fee calculators; `/methodology/`, `/changelog/`, `/about/`, `/privacy/`, `/terms/`; and a 404 page for unknown routes. Tools are free;
