...
- Public routes are `/`; `/freelance/` plus the hourly rate, project rate, retainer, markup and margin, and salary to hourly calculators; `/fees/` plus the Stripe, PayPal, Square, Etsy, eBay, Upwork, Fiverr, Kickstarter, Patreon, Ko-fi, Substack, Payhip, Gumroad, and Lemon Squeezy calculators, the KDP royalty calculator, the `/fees/stripe-vs-paypal-fees/`, `/fees/gumroad-vs-lemon-squeezy-fees/`, `/fees/upwork-vs-fiverr-fees/`, and `/fees/patreon-vs-ko-fi-fees/` comparisons, and `/fees/digital-product-platform-fees/`; `/guides/` and its guides; noindex `/embed/` copies of every calculator; `/methodology/`, `/changelog/`, `/about/`, `/privacy/`, `/terms/`; and a 404 page for unknown routes. Tools are free;
